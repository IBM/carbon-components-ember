...
function scrollRectIntoView(view, rect, startDOM) {
  ...
  for (let parent = startDOM || view.dom;;) {
    ...
    let moveX = 0,
      moveY = 0;
    if (rect.top < bounding.top + getSide$1(scrollThreshold, "top")) moveY = -(bounding.top - rect.top + getSide$1(scrollMargin, "top"));else if (rect.bottom > bounding.bottom - getSide$1(scrollThreshold, "bottom")) moveY = rect.bottom - rect.top > bounding.bottom - bounding.top ? rect.top + getSide$1(scrollMargin, "top") - bounding.top : rect.bottom - bounding.bottom + getSide$1(scrollMargin, "bottom");
...
function readName(input, index, allowHyphen) {
  while (index < input.length && (isWordCharacter(input[index]) || allowHyphen)) index += 1;
  return index;
}
function readQuotedSegment(input, index) {
  const closingQuote = input.indexOf(input[index], index + 1);
  return {
    nextIndex: skipToWhitespace(input, closingQuote === -1 ? index : closingQuote + 1)
  };
}
...
function readNamedAttribute(input, index, syntax) {
  ...
  const keyValue = readKeyValue(input, name, nameEnd);
...
function readAttribute(input, index, syntax) {
  ...
  if (isWordCharacter(prefix)) return readNamedAttribute(input, index, syntax);
  return {
    nextIndex: skipToWhitespace(input, index)
  };
}
...
var Node$1 = class Node2 extends Extendable {
  constructor(..._args) {
    super(..._args);
    this.type = "node";
  }
...
function createSuggestionFloatingUiConfig({
  ...
  const middleware = [offset({
    mainAxis: (_offset$mainAxis = offset$1.mainAxis) !== null && _offset$mainAxis !== void 0 ? _offset$mainAxis : 4,
    crossAxis: (_offset$crossAxis = offset$1.crossAxis) !== null && _offset$crossAxis !== void 0 ? _offset$crossAxis : 0
  })];
  if (flip$1) middleware.push(flip());
...
function createSuggestionView({
  ...
  return {
    update: async (view, prevState) => {
      ...
      if (!prev.active && next.active) currentState = "started";else if (prev.active && !next.active) currentState = "stopped";else if (next.active && effectiveQueryChanged) currentState = "updated";else return;
...
const activeByEditor = new WeakMap();
function registerSuggestionCommand(editor, pluginKey, command) {
  activeByEditor.set(editor, {
    command,
    pluginKey
  });
}
...
function unregisterSuggestionCommand(editor, pluginKey) {
  if (activeByEditor.get(editor)?.pluginKey === pluginKey) {
    activeByEditor.delete(editor);
  }
}
function hasActiveSuggestion(editor) {
...
function selectActiveSuggestion(editor, item) {
 ...
 * active.
 */
function dismissActiveSuggestion(editor) {
  const active = activeByEditor.get(editor);
  if (!active) {
    return false;
  }
...
function setHostOriginMeta(tr) {
  return tr.setMeta(HOST_ORIGIN_META_KEY, HOST_ORIGIN_VALUE);
}
function isHostOrigin(tr) {
  return tr.getMeta(HOST_ORIGIN_META_KEY) === HOST_ORIGIN_VALUE;
}

export { DecorationSet as D, Extension as E, Fragment as F, Mapping as M, Node$1 as N, PluginKey as P, Suggestion as S, TextSelection as T, Node$2 as a, Plugin as b, createInlineMarkdownSpec as c, dropPoint as d, Slice as e, NodeSelection as f, Decoration as g, Selection as h, isHostOrigin as i, isNodeEmpty as j, keydownHandler as k, getChangedRanges as l, mergeAttributes as m, callOrReturn as n, getExtensionField as o, isNodeSelection as p, setHostOriginMeta as q, registerSuggestionCommand as r, src_default as s, selectActiveSuggestion as t, unregisterSuggestionCommand as u, dismissActiveSuggestion as v, Editor as w, hasActiveSuggestion as x };
