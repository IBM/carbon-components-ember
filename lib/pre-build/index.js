...
class PreBuild {
  ...
  runCommand(commandOptions, rawArgs) {
    ...
    rootAddon.options = {
      babel: {
        plugins: [
          [require('@babel/plugin-syntax-decorators').default, { decoratorsBeforeExport: true }]
        ]
      },
      'ember-cli-babel': {
    ...
    let processedGts = js;
    registry.registries.js.forEach((reg) => {
      const t = reg.toTree(processedGts);
      processedGts = new MergeTrees([processedGts, t], { overwrite: true });
    });

    processedGts = new Funnel(processedGts, { include: [/\.js$/, /\.d\.ts/] })

    const treeForStyles = rootAddon.treeForStyles && rootAddon.treeForStyles(styles) || styles;

    tree = new MergeTrees([processedGts, tree, treeForStyles]);
    tree = new MergeTrees([
