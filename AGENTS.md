...
### `chat-history` family (todo #690) — no `UiShell` reuse, `OverflowMenu` already auto-flips

Ported all 13 upstream modules under `chat-history/src/` (`history-shell`,
`-header`, `-toolbar`, `-content`, `-loading`, `-delete-panel`, `-panel`,
`-panel-items`, `-panel-menu`, `-panel-item`, `-panel-item-input`,
`-search-item`) as 12 Ember components (`ChatHistoryPanelItem` covers both
`-panel-item` and its `Action` type). Files are named `chat-history-*.gts`
(not upstream's bare `history-*` basenames) specifically so
`create-files.mjs`'s filename-derived PascalCase (`ChatHistory`,
`ChatHistoryHeader`, `ChatHistoryPanelItem`, ...) comes out identical to
the hand-curated names with **zero** override entries needed in either
`create-files.mjs`'s ai-chat-prefix array or `parity-check.mjs`'s
`AI_CHAT_EXPORT_OVERRIDES` - verified by actually running
`pnpm create-autogenerated-files` and diffing (per the PR #838 CI
postmortem precedent), not just by inspection.

**Dependency question 1 (base component reuse) - no, render plain
`cds--side-nav__*` markup directly.** This addon's own `UIShell`/`Sidenav`
(`src/components/ui-shell/-sidenav.gts`) is a different, Carbon-React-parity
component: a fixed `menuItems`/`submenus` config tree for page-level
navigation, with no concept of a dynamic, renamable/deletable item list -
reusing it would mean bending its config-driven API to fit a completely
different data shape. Verified the real Lit source instead
(`@carbon/web-components`' `CDSSideNav`/`CDSSideNavItems`/`CDSSideNavMenu`/
`CDSSideNavMenuItem`, fetched from GitHub) and ported their actual render()
shapes as plain elements. Important verification step before trusting any
`cds--side-nav__*` class name: grepped the resolved `@carbon/styles`
compiled CSS directly and found `.cds--side-nav__link`'s real rules are
**all tag-qualified to `a.cds--side-nav__link`** (Carbon React's own
`SideNavLink` always renders an `<a>`) - a `<button class="cds--side-nav__
link">` (what `history-panel-item`/`history-search-item` actually render,
matching the Lit base class) gets **none** of them. This is exactly why
upstream's own `chat-history.scss` re-declares a full untagged
`.cds--side-nav__link { ... }` block from scratch rather than relying on
`@carbon/styles`; this port's own `_chat-history.scss` does the same
(adapted from `:host(...)` to plain classes, `theme.$x` to `var(--cds-x,
fallback)`, matching the established pattern). Other classes
(`.cds--side-nav__submenu`, `.cds--side-nav__menu`, `.cds--side-nav__icon`,
`.cds--side-nav__submenu-chevron`) *are* untagged in the compiled CSS and
were reused directly without re-declaring them.

**Dependency question 2 (overflow-menu flip-to-open-upward) - not needed,
this addon's `OverflowMenu` already does it.** Upstream's
`_adjustMenuPosition` in `history-panel-item.ts` is a documented workaround
for `cds-overflow-menu` not supporting an upward flip. This addon's
`OverflowMenu` is built on `ember-basic-dropdown`, whose `verticalPosition`
defaults to `'auto'` (confirmed by reading `basic-dropdown.js` directly,
not assumed) - it already flips based on available viewport space with no
extra work. One real, disclosed divergence: upstream flips relative to the
nearest `cds-aichat-history-content` ancestor's bounds, this port flips
relative to the viewport, so in a history panel shorter than the viewport
the menu can still open downward and overflow the panel while fitting the
viewport fine (same class of gap as `positionCalendarWithinAppendTo`'s
missing horizontal clamping in `DatePicker`).

**Two structural departures from upstream's DOM, both disclosed in
`ChatHistoryPanelItem`'s class doc:** (1) upstream nests its
`cds-overflow-menu` trigger *inside* the same `<button>` that selects the
row, needing an explicit `composedPath()`-based click guard
(`isOverflowMenuClick`) to stop a menu click from also selecting the item.
This port's `OverflowMenu` renders its trigger as a sibling of the row
button (both to avoid invalid nested-interactive-content HTML, and because
`ember-basic-dropdown` wormholes the menu's *content* to `document.body`
regardless - only the *trigger* stays in-place, and moving it out of the
button removes the need for the guard entirely). (2) upstream also listens
for `Enter` at the host keydown level with an explicit `preventDefault()` +
re-dispatch, to guarantee consistent event detail shape; this port's row
*is* a real `<button>`, which activates via Enter/Space natively with the
correct detail already in scope, so the extra handling isn't needed.

**`Action.onClick` in upstream's own type is dead code** - its actual
`render()` wires every `cds-overflow-menu-item` to one shared
`_handleMenuItemClick` that dispatches a single `history-item-menu-action`
event with the action's text; `action.onClick` (declared in the `Action`
interface) is never invoked anywhere. Matches this doc's own "match
behavior over intent" principle: `ChatHistoryItemAction` omits `onClick`
entirely (an accepted-but-ignored arg is worse than an absent one, per the
already-established `PromptLine` rich-mode precedent) and the real behavior
ships as a single `@onMenuAction` callback instead.

**`@showActions`/`@parentMenuExpanded` propagate via yielded pre-bound
args (`WithBoundArgs`, the `Layer`/`ChainOfThought` pattern), not
upstream's `MutationObserver`-on-an-attribute plus a
`cds-side-nav-menu-toggled` event listener** (`history-panel-item.ts`'s
`connectedCallback`) - `ChatHistoryPanel` yields `ChatHistoryPanelItems`
pre-bound with `@showActions`, which yields both `ChatHistoryPanelItem` and
`ChatHistoryPanelMenu` pre-bound with it in turn, and `ChatHistoryPanelMenu`
yields `ChatHistoryPanelItem` pre-bound with both `@showActions` and its
own live `@parentMenuExpanded`. No DOM observer needed on either side.

**`rename` doesn't cleanly fit the standard Case B convention, so it isn't
forced into one.** Upstream's own `history-panel-item.ts` mutates its own
...
manager cuts): `history-shell.ts`'s `_handleHistoryDeleteConfirm` (queries
sibling `history-panel-item` elements by id, double-`requestAnimationFrame`
focus, synthesizes a `history-item-selected` event on whichever item comes
next) isn't ported - `ChatHistoryDeletePanel`'s `@onConfirm` only passes
`@itemId` back, and a consumer wires its own list/focus logic from there.
Arrow Up/Down cycling between overflow-menu items
(`_handleMenuItemKeyDown`) also isn't ported - `OverflowMenu` doesn't
provide it, and Tab still reaches every item.

**A real, pre-existing bug in `AiChatChatButton` was uncovered (and fixed)
by this being the first caller to pass `@kind='danger'` anywhere in this
initiative:** `Button`'s `@type='danger'` (or `@danger={{true}}`) doesn't
just add styling - it also opts into `Button`'s own built-in native
confirm-dialog-before-click flow (`showDialog`/`onButtonClick`), which
renders via `{{#in-element this.dialogManager.destinationElement}}` into a
`#carbon-components-dialog-id` mount point that doesn't exist in a plain
component test (or in this addon's demo pages) - `AiChatChatButton @kind=
'danger'` crashed outright with "cannot pass a null or undefined
destination element to in-element". Upstream's real `cds-aichat-button`
`kind="danger"` is a pure color variant with no such behavior. Fixed in
`chat-button.gts`: `buttonType` now excludes `'danger'` from what's passed
to `Button`'s `@type` (alongside the existing `'ghost'`/`'tertiary'`
exclusions), and the `cds--btn--danger` class is applied directly instead -
same styling, without opting into the unrelated dialog machinery.
`ChatHistoryDeletePanel`'s own Delete button is exactly the case this
matters for: that component *is* the confirmation UI, so a second, nested,
unrelated confirm dialog would have been a broken double-confirmation even
if the crash weren't there. Worth checking for this same trap before any
future `AiChatChatButton @kind='danger'` (or a raw `Button @type='danger'`)
use inside another already-a-confirmation AI Chat surface.

Full checklist green: `pnpm exec glint`, addon `build:js`/`build:types`,
`pnpm run lint` (0 errors, only pre-existing unrelated warnings), full
`test-app` suite 1005/1008 (33 new tests; the 3 "not ok" were the two
already-documented pre-existing `Grid` white/dark theme snapshot flakes
plus their `js-reporters` module-aggregate double-count, not this PR's own
code - reproduced identically on a clean rerun with no relevant diff). Real
`DOCS_URL=versions/main pnpm build` (12 new lazy `.gjs-*.js` chunks
confirmed) served locally (custom SPA-fallback static server per
`project_docs_app_local_browser_verification`) and driven with Playwright:
all 12 new docs pages render inside a real shadow root with zero page
errors, plus a full end-to-end interaction pass on the flagship
`chat-history` demo (select an item, open the overflow menu, rename via
