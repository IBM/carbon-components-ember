import { b as LRParser, E as ExternalTokenizer, C as ContextTracker, s as styleTags, t as tags, L as LanguageSupport, a as LRLanguage, d as foldNodeProp, m as foldInside, i as indentNodeProp, k as continuedIndent } from './index-DUpYiFjb.js';
import { d as defineCSSCompletionSource } from './index-DxA39neW.js';
...
const spaces = new ExternalTokenizer((input, stack) => {
  if (stack.dialectEnabled(Dialect_indented)) {
    ...
    if (input.next < 0 && stack.canShift(eof)) {
      ...
      if (input.next == newlineChar || startOfComment(input)) input.acceptToken(blankLineStart, -spaces);else if (spaces) input.acceptToken(whitespace);
...
const indentation = new ExternalTokenizer((input, stack) => {
  ...
  if (prev == newlineChar) {
    ...
    if (depth != cDepth && input.next != newlineChar && !startOfComment(input)) {
      if (depth < cDepth) input.acceptToken(dedent, -depth);else input.acceptToken(indent);
    }
  }
});
...
const interpolationEnd = new ExternalTokenizer(input => {
  if (input.next == braceR) {
    ...
    if (input.next == hash && input.peek(1) == braceL) input.acceptToken(InterpolationContinue, 2);else input.acceptToken(InterpolationEnd);
  }
});
