...
class SourceSpan {
  ...
  toSlice(expected) {
    ...
    assert$2(expected === undefined || expected === chars, `unexpectedly found ${JSON.stringify(chars)} when slicing source, ` + `but expected ${JSON.stringify(expected)}`);
    return new SourceSlice({
      loc: this,
...
const debug$1 = {};
{
  ...
  debug$1.setTrackingTransactionEnv = env => Object.assign(TRANSACTION_ENV, env);
  debug$1.beginTrackingTransaction = _debugLabel => {
  ...
  debug$1.endTrackingTransaction = () => {
  ...
  debug$1.resetTrackingTransaction = () => {
    let stack = '';
    if (TRANSACTION_STACK.length > 0) {
      stack = debug$1.logTrackingStack(TRANSACTION_STACK[TRANSACTION_STACK.length - 1]);
    }
   ...
   */
  debug$1.runInTrackingTransaction = (fn, debugLabel) => {
    debug$1.beginTrackingTransaction(debugLabel);
    let didError = true;
    try {
      let value = fn();
      didError = false;
  ...
  let makeTrackingErrorMessage = (transaction, obj, keyName) => {
    ...
    message.push(debug$1.logTrackingStack(transaction));
    message.push(`Stack trace for the update:`);
    return message.join('\n\n');
  };
  debug$1.logTrackingStack = transaction => {
    let trackingStack = [];
  ...
  debug$1.markTagAsConsumed = _tag => {
    ...
    if (!subtag || !debug$1.markTagAsConsumed) return;
    if (Array.isArray(subtag)) {
      subtag.forEach(debug$1.markTagAsConsumed);
    } else {
      debug$1.markTagAsConsumed(subtag);
    }
  };
  debug$1.assertTagNotConsumed = (tag, obj, keyName) => {
    ...
    // occurred.
    try {
      assert$1(false, makeTrackingErrorMessage(transaction, obj, keyName));
    } catch (e) {
      if (hasStack(e)) {
...
class MonomorphicTagImpl {
  ...
  static dirtyTag(tag, disableConsumptionAssertion) {
    ...
    if (disableConsumptionAssertion !== true) {
      ...
      unwrap(debug$1.assertTagNotConsumed)(tag);
    }
