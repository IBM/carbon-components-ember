import { b as base, s as shift, k as keyName } from './index-D2CeE0KS.js';
      ...
      if (enter && cursor.firstChild()) {
        ...
        for (;;) {
          ...
          if (overlay && ! --overlay.depth) {
            ...
            if (ranges.length) {
              checkRanges(ranges);
              this.inner.splice(overlay.index, 0, new InnerParse(overlay.parser, overlay.parser.startParse(this.input, enterFragments(overlay.mounts, ranges), ranges), overlay.ranges.map(r => new Range$1(r.from - overlay.start, r.to - overlay.start)), overlay.bracketed, overlay.target, ranges[0].from));
            }
...
class EditorSelection {
  ...
  */
  static cursor(pos, assoc = 0, bidiLevel, goalColumn) {
    return SelectionRange.create(pos, pos, (assoc == 0 ? 0 : assoc < 0 ? 8 /* RangeFlag.AssocBefore */ : 16 /* RangeFlag.AssocAfter */) | (bidiLevel == null ? 7 : Math.min(6, bidiLevel)), goalColumn);
  }
  /**
  ...
  */
  static range(anchor, head, goalColumn, bidiLevel, assoc) {
    ...
    if (assoc) flags |= assoc < 0 ? 8 /* RangeFlag.AssocBefore */ : 16 /* RangeFlag.AssocAfter */;
    return head < anchor ? SelectionRange.create(head, anchor, flags | 32 /* RangeFlag.Inverted */, goalColumn) : SelectionRange.create(anchor, head, flags, goalColumn);
  }
  /**
      ...
      for (let i = 0, off = 0; i < tile.children.length && off <= pos; i++) {
        ...
        if (end >= pos) {
          if (child.isComposite()) {
            scan(child, pos - off);
          } else if ((!after || after.isHidden && (side > 0 && !(after.flags & 32 /* TileFlag.After */) || forCoords && onSameLine(after, child))) && (end > pos || child.flags & 32 /* TileFlag.After */ && side <= 1)) {
            after = child;
            afterOff = pos - off;
...
class DocView {
  ...
  scrollIntoView(target) {
    ...
    let rect = this.coordsAt(range.head, range.assoc || (range.head > range.anchor ? -1 : 1)),
      other;
    if (!rect) return;
  ...
  for (let i = 0; i < sel.ranges.length; i++) {
    ...
    if (range.empty) {
      ...
      if (from != range.from || to != range.to) {
        if (range.undirectional) updated = EditorSelection.undirectionalRange(range.from, range.to);else updated = EditorSelection.range(range.from == range.anchor ? from : to, range.from == range.head ? from : to);
      }
    }
    if (updated) {
          ...
          if (this.viewState.editorHeight) {
            if (this.viewState.scrollTarget) {
              ...
              if ((diff > 1 || diff < -1) && !(browser.ios && this.inputState.lastIOSMomentumScroll > Date.now() - 100) && (scroll == this.scrollDOM || this.hasFocus || Math.max(this.inputState.lastWheelEvent, this.inputState.lastTouchTime) > Date.now() - 100)) {
                ...
                if (!scroll) this.win.scrollBy(0, diff);else if (scrollAnchorPos < 0) scroll.scrollTop = scroll.scrollHeight;else scroll.scrollTop += diff;
                scrollAnchorHeight = -1;
                continue;
              }
            }
...
function createDialog(view, config, result) {
  ...
  return {
    ...
    mount: () => {
      if (config.focus) {
        let focus;
        if (typeof config.focus == "string") focus = content.querySelector(config.focus);else focus = content.querySelector("input") || content.querySelector("button");
...
export { countColumn as $, Annotation as A, NodeProp as B, CharCategory as C, Decoration as D, EditorSelection as E, Facet as F, Tree as G, Language as H, IterMode as I, defineLanguageFacet as J, foldNodeProp as K, LanguageDescription as L, MapMode as M, NodeSet as N, indentNodeProp as O, Prec as P, languageDataProp as Q, RangeSet as R, StreamLanguage as S, Transaction as T, continuedIndent as U, ViewPlugin as V, WidgetType as W, delimitedIndent as X, foldInside as Y, IndentContext as Z, getIndentation as _, LanguageSupport as a, indentString as a0, matchBrackets as a1, findClusterBreak as a2, ChangeSet as a3, ChangeDesc as a4, getIndentUnit as a5, getPanel as a6, activateHover as a7, showPanel as a8, RangeSetBuilder as a9, crelt as aa, hoverTooltip as ab, EditorState as ac, ParseContext as ad, bracketMatchingHandle as ae, NodeWeakMap as af, flatIndent as ag, sublanguageProp as ah, showDialog as ai, runScopeHandlers as aj, lineNumbers as ak, highlightActiveLineGutter as al, highlightSpecialChars as am, foldGutter as an, drawSelection as ao, dropCursor as ap, indentOnInput as aq, syntaxHighlighting as ar, defaultHighlightStyle as as, bracketMatching as at, rectangularSelection as au, crosshairCursor as av, highlightActiveLine as aw, foldKeymap as ax, Compartment as ay, HighlightStyle as az, EditorView as b, codePointAt as c, codePointSize as d, StateField as e, fromCodePoint as f, StateEffect as g, RangeValue as h, Text as i, getTooltip as j, showTooltip as k, combineConfig as l, indentUnit as m, keymap as n, logException as o, Direction as p, foldService as q, LRLanguage as r, syntaxTree as s, parseMixed as t, styleTags as u, tags as v, Tag as w, Parser as x, NodeType as y, DefaultBufferLength as z };
